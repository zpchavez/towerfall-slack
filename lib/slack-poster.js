...
var fixGif        = require('towerfall-gif-fixer');
...
SlackPoster.prototype.postReplay = function(file, callback) {
    ...
    const fixedFile = file.substr(0, file.length - 4) + `-fixed.gif`;
    return fixGif(file, fixedFile).then((goodFile) => {
      slackUploader.uploadFile(
          {
              file     : fs.createReadStream(goodFile),
